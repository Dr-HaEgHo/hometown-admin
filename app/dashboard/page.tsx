  ...
  return (
    <div className={`w-[25%] aspect-square rounded-[10px] shadow flex flex-col items-center justify-center`} style={{
      ...
      <span className='text-[30px] 2xl:text-xs text-primary font-semibold'>
        {
          loading === true ? <Loader /> : counts? (<>
            {item.id === 1 && new Intl.NumberFormat("NG", {
            }).format(counts?.total_farmers ? counts.total_farmers : 0) }
            {item.id === 2 && new Intl.NumberFormat("NG", {
            }).format(counts?.total_customers ? counts.total_customers : 0) }
            {item.id === 3 && new Intl.NumberFormat("NG", {
            }).format(counts?.total_input_dealers? counts.total_input_dealers : 0)}
            {item.id === 4 && new Intl.NumberFormat("NG", {
            }).format(counts?.total_agents ? counts.total_agents : 0)  
            }
          </>) : 0  
        }
      </span>
